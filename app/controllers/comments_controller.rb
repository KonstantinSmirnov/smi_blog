class CommentsController < ApplicationController
  def create
    ...
    if cookies[:opened_article_id] == params[:article_id] && ( Time.now.to_i - cookies[:article_opened_at].to_i) > 5
      ...
      respond_to do |format|
        format.js { render 'create_fail.js.erb', :locals => { :text => "Antispam filter requires to wait #{5 - Time.now.to_i + cookies[:article_opened_at].to_i} more seconds after Article is opened." } }
      end
    end
  end
